...
// UPDATE book - comprehensive version
router.put('/update-book/:bookId', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Handle AcquisitionDate with proper formatting
    ...
    // Add BookID to values for WHERE clause
    values.push(bookId);
...
router.post('/add-book', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const [existingBooks] = await db.execute('SELECT BookID FROM books WHERE ISBN = ?', [ISBN]);
